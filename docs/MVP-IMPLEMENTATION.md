...
- [x] **Secure admission:** the deployment key derives a pinned Deployment CA, inter-Node QUIC requires CA-signed certificates in both directions, and an opaque `up://` Join Link provisions the key while Raft consumes its expiring single-use token before membership changes.
