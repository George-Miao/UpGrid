...
- [x] **Secret protection:** Secrets use randomized authenticated encryption before Raft replication. Bootstrap creates a durable deployment key, each Join Link provisions that same key, and lifecycle/tamper tests cover the boundary.
- [x] **Secure admission:** the deployment key derives a pinned Deployment CA, inter-Node QUIC requires CA-signed certificates in both directions, and an opaque `ups://` Join Link provisions the key while Raft consumes its expiring single-use token before membership changes.
