...
- Node admission uses one opaque `up://` Join Link containing a single-use expiring token and the deployment material needed for a pinned Deployment CA and mutual TLS. A new Node joins with only `--join <link>` in addition to its ordinary Node configuration.
