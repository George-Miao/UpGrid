...
export function renderAccessPanels(identities: Identity[], tokens: ApiToken[], currentIdentityId: string | undefined, newToken: string, saving: boolean, actions: AuthActions) {
  return html`
    <div class="page-columns access-panels">
      <section class="panel" aria-label="Operator Identities">
        ...
        <form class="choice compact-form" @submit=${actions.createIdentity}>
          <h3>Add administrator</h3>
          <label>Username<input name="username" required /></label>
          <label>Password<input name="password" type="password" minlength="12" autocomplete="new-password" required /></label>
          <button class="button" type="submit" ?disabled=${saving}>Add identity</button>
        </form>
      </section>
