...
import "iconify-icon";
import "@/view/setup-flow.ts";
import "@/component/http-assertion-editor.ts";
import type { ClusterMember, PublicStatusTarget, Target, TrashedTarget } from "@/app/api.ts";
import { AppController } from "@/app/app-controller.ts";
import { type Section, sectionPaths, themeIcons } from "@/app/app-state.ts";
...
import { type AuthActions, renderApiTokensPage, renderChangePassword, renderLogin, renderManagePage, renderUsersPage } from "@/view/auth.ts";
...
export class UpgridApp extends AppController {
  ...
  protected render() {
    ...
    if (this.authReady && !this.setupMode && !this.session && !this.publicStatus) {
      return html`${renderLogin(this.live, this.saving, this.error, authActions)}${renderFooter()}`;
    }
    ...
    return html`
                ...
                : this.activeSection === "trash"
                  ...
                  : this.activeSection === "manage"
                    ? renderManagePage(
                        ...
                        this.saving,
                        this.error,
                        (event) => void this.updateSettings(event),
                        () => (this.error = ""),
                      )
                    : this.activeSection === "changePassword"
                      ...
                      : this.activeSection === "users"
                        ? renderUsersPage(this.identities, this.session?.identity_id, this.editingIdentity, this.saving, this.error, authActions)
                        : renderApiTokensPage(this.apiTokens, this.newApiToken, this.saving, this.error, authActions)
        }
      </main>${renderFooter()}
      ${renderTargetForm(this.channels, this.secrets, this.saving, this.targetError, {
        backdrop: (event) => this.dismissOnBackdrop(event),
        close: () => this.closeTargetDialog(),
        create: (event) => void this.createTarget(event),
        changed: () => (this.targetError = ""),
      })}
      ${
        this.selected
          ? renderTargetDetail(this.selected, this.targetHistory, this.historyLoading, this.saving, this.detailDirty, this.detailTab, this.cluster?.members ?? [], this.channels, this.secrets, this.targetError, {
              backdrop: (event) => this.dismissOnBackdrop(event),
      ...
      <dialog id="secret-dialog" aria-labelledby="secret-title" @click=${this.dismissOnBackdrop}>
        ...
        <form @submit=${this.createSecret} @input=${() => (this.error = "")}>
          ...
          <div class="dialog-actions"><button class="button secondary" type="button" @click=${() => this.closeDialog("secret-dialog")}>Cancel</button>${renderFormSubmit({ label: "Create secret", busy: this.saving, error: this.error })}</div>
        </form>
      </dialog>
      ...
      <dialog id="join-dialog" aria-labelledby="join-title" @click=${this.dismissOnBackdrop}>
        <div class="dialog-head"><div class="title-with-help"><h2 id="join-title">Join token created</h2>${renderHelpTooltip("join-token-url-help", "About join token credentials", "This URL contains cluster credentials. Revoke the token when no longer needed.")}</div></div>
  ...
  private renderPublicStatusPage(targets: PublicStatusTarget[]) {
    ...
    return html`
      <main class="shell">
        ...
        ${renderCard({
          title: "Targets",
          label: "Public target status",
          metadata: `${targets.length} monitored`,
          ...
          content: html`
            ${
              targets.length
                ...
                : html`<upgrid-empty-state>No targets are configured</upgrid-empty-state>`
            }
          `,
        })}
      </main>${renderFooter()}`;
  ...
  private renderOverview(visibleTargets: Target[], up: number, down: number, pending: number) {
    ...
    return html`
      ...
      <section class="overview-top">
        ...
        ${renderCard({
          ...
          actions: [
            ...
            { key: "add-secret", label: "Add secret", variant: "secondary", onClick: () => this.showDialog("secret-dialog") },
          ],
          content: html`
            ${
              this.secrets.length
      ...
      ${renderCard({
        title: "Targets",
        metadata: `${this.targets.length} configured`,
        content: html`
          <div class="toolbar">
  ...
  private renderTrashPage() {
    return html`
      ...
      ${renderCard({
        ...
        label: "Trashed targets",
        tooltip: {
          id: "trash-retention-help",
          label: "About deleted target retention",
          message: "Settings and history remain recoverable until the retention deadline.",
        },
        metadata: `${this.trashedTargets.length} stored`,
        content: html`${this.trashedTargets.length ? this.trashedTargets.map((target) => this.renderTrashedTarget(target)) : html`<upgrid-empty-state>Trash is empty</upgrid-empty-state>`}`,
      })}
    `;
  }
  ...
  private renderClusterPage() {
    return html`
      ...
      ${renderCard({
        ...
        tooltip: {
          id: "nodes-removal-help",
          label: "About removing nodes",
          message: "Drain healthy nodes before removal. Replace failed nodes only after confirming the old process is permanently stopped.",
        },
        metadata: `${this.cluster?.members.length ?? 0} members`,
        content: html`
          ${this.cluster?.members.map((member) => this.renderClusterMember(member))}
          ${this.cluster?.members.length ? nothing : html`<upgrid-empty-state>Cluster topology unavailable</upgrid-empty-state>`}
        `,
      })}
      ${renderCard({
        title: "Join tokens",
        metadata: `${this.joinTokens.length} stored`,
        content: html`
          ${
            this.joinTokens.length
              ...
              : html`<upgrid-empty-state>No join tokens</upgrid-empty-state>`
          }
        `,
      })}
      </div>
