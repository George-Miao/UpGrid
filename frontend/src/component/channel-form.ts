...
@customElement("notification-channel-form")
export class NotificationChannelForm extends LitElement {
  ...
  protected render() {
    ...
    return html`<form @submit=${this.save} @input=${this.formChanged}>
      ...
      ${this.renderFields()}
      <label class="switch"><span>Default channel</span><input class="switch-control" name="default" type="checkbox" role="switch" .checked=${this.isDefault} ?disabled=${busy} @change=${(event: Event) => (this.isDefault = (event.target as HTMLInputElement).checked)} /></label>
  ...
  private renderFields() {
    ...
    if (this.kind === "telegram") {
      return html`
        <label><span class="title-with-help">Bot token ${renderHelpTooltip("telegram-token-help", "About Telegram bot token storage", this.channel ? "Get a replacement token from Telegram's @BotFather. Leave this blank to keep the automatically managed secret, or enter the replacement token." : "Get a bot token from Telegram's @BotFather. Creating the channel encrypts it as an automatically managed secret. Test sends use the entered value without storing it.")}</span><input name="bot_token" type="password" autocomplete="off" placeholder=${this.channel ? "Leave blank to keep current token" : ""} data-test-required ?required=${this.channel === undefined} /></label>
