    ...
    `}renderOverview(t,e,i,s){const n=this.targets.filter(l=>this.selectedIds.has(l.id)),r=n.some(l=>!l.paused),a=n.some(l=>l.paused),o=this.secrets.filter(l=>!l.referenced);return c`
      ...
      <section class="overview-top">
        ...
        <section class="panel" aria-label="Secrets">
          <div class="panel-head"><div class="title-with-help"><h2>Secrets</h2>${ge("secrets-help","About reusable Secrets","Reusable Secrets are encrypted and write-only. UpGrid reports whether each Secret is referenced by an active or trashed Target or a Notification Channel.")}</div><div class="actions">${o.length?c`<button class="button danger" ?disabled=${this.saving} @click=${()=>this.cleanupSecrets()}>Delete unused (${o.length})</button>`:f}<button class="button secondary" @click=${()=>this.showDialog("secret-dialog")}>Add secret</button></div></div>
