                ...
                <div class="resource alert-resource">
                  <div class="alert-summary">
                    <div class="channel-title">
                      ...
                      ${o.acknowledged_at_ms===null?m:c`<span class="badge">acknowledged</span>`}
                    </div>
                    <code>${o.channel_name} · ${new Date(o.scheduled_at_ms).toLocaleString()}</code>
                    <span class="meta">${qs(o)}</span>
                  </div>
                  <div class="alert-actions">
                    ${o.delivery==="failed"?c`<button class="button secondary" ?disabled=${n} @click=${()=>r.retry(o)}>Retry</button>`:m}
                    ${o.acknowledged_at_ms===null?c`<button class="button secondary" ?disabled=${n} @click=${()=>r.acknowledge(o)}>Acknowledge</button>`:m}
                  </div>
                </div>
  ...
  `}function zs(t,e,i){return c`
    ...
    <div class="page-columns access-panels">
      ...
      <section class="panel" aria-label="API Tokens">
        ...
        ${s?c`<div class="notice token-value" role="status"><strong>Copy this token now.</strong><code>${s}</code><button class="button secondary" @click=${r.dismissToken}>Dismiss</button></div>`:m}
    ...
    </div>`}const Js={width:24,height:24,body:'<g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"><circle cx="12" cy="12" r="10"/><path d="M12 16v-4m0-4h.01"/></g>'},Bs=me`
...
`;function ge(t,e,i){return c`
  ...
  `}const Xt={http:"https://example.com/health",tcp:"database.internal:5432",dns:"service.internal",icmp:"192.0.2.10",tls:"example.com:443"};function ei(t,e){const i=t.elements.namedItem("url");i&&(i.placeholder=Xt[e],i.type=e==="http"?"url":"text");const s=t.querySelector("[data-http-options]");s&&(s.hidden=e!=="http");const n=t.elements.namedItem("method");n&&(n.disabled=e!=="http",n.disabled&&(n.value="GET"))}function Ws(t){const e=t.currentTarget;e.form&&ei(e.form,e.value)}function Qs(t){const e=t.currentTarget;queueMicrotask(()=>ei(e,"http"))}function Ys(t,e,i,s){return c`
    ...
    </dialog>`}function Zs(t,e,i,s,n,r,a,o,l){const d=t.kind==="node",p=t.kind==="http",u=t.accepted_statuses.map(h=>h.start===h.end?h.start:`${h.start}-${h.end}`).join(","),y=t.history.slice(0,30).reverse(),$=Math.max(1,...y.map(h=>h.latency_ms)),w=e?.items??[],S=w.reduce((h,A)=>h+A.samples,0),_=w.reduce((h,A)=>h+A.successes,0),J=w.reduce((h,A)=>h+A.latency_total_ms,0),C=S?`${(_/S*100).toFixed(2)}%`:"—",T=new Map(r.map(h=>[h.id,h.name])),k=h=>new Date(h).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),D=h=>h>=1e3?`${(h/1e3).toFixed(h>=1e4?0:1)} s`:`${Math.round(h)} ms`,x=S?D(J/S):"—";return c`
      ...
      <div class="target-wrap">
        ...
        <button class=${`target ${e?"node-target":""}`} aria-label=${t.name} @click=${()=>this.openTarget(t)}>
          ...
          <div class="target-side">
            ${n.length?c`<div class="mini-chart" aria-hidden="true">${n.map(o=>c`<i class="mini-bar ${o.succeeded?"up":"down"}" style=${`height: ${Math.max(12,o.latency_ms/r*100)}%`}></i>`)}</div>`:m}
            <div class="latency">
