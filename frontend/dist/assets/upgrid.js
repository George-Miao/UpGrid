...
`;function Z(t,e,i){return c`
  ...
  `}function Xs(t,e){return t==="webhook"?c`<label
    ...
    /></label>`:t==="telegram"?c`
      <label
        ><span class="title-with-help"
          >Bot token
          ${Z("telegram-token-help","About Telegram bot token storage",e?"Leave this blank to keep the automatically managed Secret, or enter a replacement token.":"Creating the Channel encrypts this token as an automatically managed Secret. Test sends use the entered value without storing it.")}</span
        ><input
          name="bot_token"
          type="password"
          autocomplete="off"
    ...
    <div class="form-field">
      <div class="title-with-help">
        ...
        ${Z("smtp-password-help","About SMTP password storage",e?"Leave this blank to keep the automatically managed Secret. Clear the username to disable authentication.":"Enter a username and password together to enable authentication. The password is encrypted as an automatically managed Secret.")}
      </div>
      <input
        id="smtp-password"
        name="password"
  ...
  `}const ti={http:"https://example.com/health",tcp:"database.internal:5432",dns:"service.internal",icmp:"192.0.2.10",tls:"example.com:443"};function Ye(t,e){t.querySelectorAll("[role='tab']").forEach(i=>{const s=i.dataset.tab===e;i.setAttribute("aria-selected",String(s)),i.tabIndex=s?0:-1}),t.querySelectorAll("[role='tabpanel']").forEach(i=>{i.hidden=i.dataset.panel!==e})}function ii(t,e){const i=t.elements.namedItem("url");i&&(i.placeholder=ti[e],i.type=e==="http"?"url":"text"),t.querySelectorAll("[data-http-only]").forEach(o=>{o.hidden=e!=="http"});const s=t.querySelector("[role='tab'][aria-selected='true']")?.dataset.tab??"general",a=t.querySelector("[data-tab='assertions']");a&&(a.disabled=e!=="http"),Ye(t,e!=="http"&&s==="assertions"?"general":s);const n=t.elements.namedItem("method");n&&(n.disabled=e!=="http",n.disabled&&(n.value="GET"))}function sa(t){const e=t.currentTarget;e.form&&ii(e.form,e.value)}function le(t){const e=t.currentTarget;e.form&&e.dataset.tab&&Ye(e.form,e.dataset.tab)}function aa(t){const e=t.currentTarget;queueMicrotask(()=>{ii(e,"http"),Ye(e,"general")})}function na(t,e,i){return c`
    ...
    </dialog>`}function ra(t,e,i,s,a,n,o,r,l,d){const u=t.kind==="node",p=t.kind==="http",y=t.accepted_statuses.map(h=>h.start===h.end?h.start:`${h.start}-${h.end}`).join(","),x=t.history.slice(0,30).reverse(),k=Math.max(1,...x.map(h=>h.latency_ms)),C=e?.items??[],$=C.reduce((h,P)=>h+P.samples,0),B=C.reduce((h,P)=>h+P.successes,0),E=C.reduce((h,P)=>h+P.latency_total_ms,0),S=$?`${(B/$*100).toFixed(2)}%`:"—",_=new Map(o.map(h=>[h.id,h.name])),q=h=>new Date(h).toLocaleString(void 0,{month:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}),w=h=>h>=1e3?`${(h/1e3).toFixed(h>=1e4?0:1)} s`:`${Math.round(h)} ms`,T=$?w(E/$):"—",I=[{id:"details",label:"Details"},{id:"general",label:"General"},...p?[{id:"assertions",label:"Assertions"}]:[],...u?[]:[{id:"evaluation",label:"Evaluation"},{id:"notifications",label:"Notifications"}]],A=I.some(({id:h})=>h===n)?n:"details";return c`
    <dialog id="detail-dialog" aria-labelledby="target-detail-title" @click=${d.backdrop}>
      <div class="dialog-head target-dialog-head">
        ...
        <button class="button secondary icon-button dialog-close" type="button" aria-label=${`Close ${u?"Node":"Target"} details`} title="Close" @click=${d.close}><iconify-icon .icon=${jt} aria-hidden="true"></iconify-icon></button>
      </div>
      <form id="detail-form" class="detail-form" @submit=${d.update} @input=${d.changed}>
        ...
        ${u?g:c`
            ...
            `}
        ${A==="details"?u?g:c`<div class="dialog-actions"><div class="danger-actions">
    ...
    `}renderTrashPage(){return c`
      ...
      <section class="panel" aria-label="Trashed Targets">
        <div class="panel-head"><div class="title-with-help"><h2>Deleted Targets</h2>${Z("trash-retention-help","About deleted Target retention","Settings and history remain recoverable until the retention deadline.")}</div><span class="meta">${this.trashedTargets.length} stored</span></div>
    ...
    `}renderClusterPage(){return c`
      ...
      <section class="panel" aria-label="Cluster topology">
        <div class="panel-head"><div class="title-with-help"><h2>Nodes</h2>${Z("nodes-removal-help","About removing Nodes","Drain healthy Nodes before removal. Replace failed Nodes only after confirming the old process is permanently stopped.")}</div><span class="meta">${this.cluster?.members.length??0} members</span></div>
