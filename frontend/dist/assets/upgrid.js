  ...
  `}function Jt(t,e=[],i=!0){return u`
    ...
    <dialog id="detail-dialog" aria-labelledby="target-detail-title" @click=${r.backdrop}>
      <div class="dialog-head">
        ...
        <button class="button secondary icon-button dialog-close" type="button" aria-label=${`Close ${o?"Node":"Target"} details`} title="Close" @click=${r.close}><iconify-icon .icon=${kt} aria-hidden="true"></iconify-icon></button>
      </div>
      <form @submit=${r.update} @input=${r.changed}>
        ...
        <div class="dialog-actions">
          ${o?f:u`<div class="danger-actions">
            ...
            <button class=${`button ${t.paused?"success":"warning"} icon-button`} type="button" aria-label=${t.paused?"Resume evaluations":"Pause evaluations"} title=${t.paused?"Resume evaluations":"Pause evaluations"} @click=${()=>r.pause(!t.paused)}><iconify-icon .icon=${t.paused?_t:wt} aria-hidden="true"></iconify-icon></button>
          </div>`}
    ...
    </dialog>`}var Ps=Object.getOwnPropertyDescriptor,Is=(t,e,i,n)=>{for(var s=n>1?void 0:n?Ps(e,i):e,r=t.length-1,o;r>=0;r--)(o=t[r])&&(s=o(s)||s);return s};let Te=class extends ks{render(){const t=this.targets.filter(o=>o.availability==="up").length,e=this.targets.filter(o=>o.availability==="down").length,i=this.alerts.filter(o=>o.delivery==="pending").length,n=ws(this.targets,this.live),s=["overview","alerts","cluster"],r=this.targets.filter(o=>`${o.name} ${o.url}`.toLowerCase().includes(this.search.toLowerCase())).filter(o=>this.statusFilter==="all"?!0:this.statusFilter==="paused"?o.paused:o.availability===this.statusFilter).sort((o,a)=>this.sort==="status"&&o.availability.localeCompare(a.availability)||o.name.localeCompare(a.name));return this.setupMode&&this.setup?u`
