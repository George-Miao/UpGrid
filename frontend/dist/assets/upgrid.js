    ...
    /></label>`:t==="telegram"?c`
      <label
        ><span class="title-with-help"
          >Bot token
          ${H("telegram-token-help","About Telegram bot token storage",e?"Get a replacement token from Telegram's @BotFather. Leave this blank to keep the automatically managed Secret, or enter the replacement token.":"Get a bot token from Telegram's @BotFather. Creating the Channel encrypts it as an automatically managed Secret. Test sends use the entered value without storing it.")}</span
        ><input
          name="bot_token"
          type="password"
          autocomplete="off"
