...
impl Form {
    ...
    /// Consume this instance and transform into an instance of Body for use in
    /// a request.
    ...
    /// Generate a hyper::Body stream for a single Part instance of a Form
    /// request.
...
impl Part {
    ...
    /// Makes a new parameter from an arbitrary stream with a known length. This
    /// is particularly useful when adding something like file contents as a
    /// stream, where you can know the content length beforehand.
...
impl<P: PartProps> FormParts<P> {
    ...
    // The length should be predictable if only String and file fields have been
    // added, but not if a generic reader has been added;
    pub(crate) fn compute_length(&mut self) -> Option<u64> {
        let mut length = 0u64;
        ...
        // If there is at least one field there is a special boundary for the very last
        // field.
