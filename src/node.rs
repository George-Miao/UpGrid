...
impl Node {
    ...
    async fn build(
        ...
        let controller = Controller::new(endpoint.clone(), deployment_key_fingerprint);
        ...
        let server_handle = spawn(async move {
            while let Some(incoming) = endpoint.wait_incoming().await {
                ...
                debug!("incoming connection dispatched");
            }
        ...
        E: std::error::Error + Send + Sync + 'static,
    {
        ...
        let client = self.controller.get_client(&remote).await?;
        ...
        let result = match result {
            Err(JoinError::Raft(RaftError::APIError(ClientWriteError::ForwardToLeader(
                ...
                .controller
                .get_client(&forward.leader_node.expect("checked above"))
                .await?
                .ask_to_join(Context::current(), self.id.clone(), token.to_owned())
                .await
                .context(RpcSnafu)?,
    ...
    pub async fn write(&self, request: Req) -> Result<Res, String> {
        ...
        loop {
            ...
            let result = if let Some(forwarded_to) = leader.take() {
                ...
                let client =
                    match timeout(attempt_timeout, self.controller.get_client(&forwarded_to)).await
                    {
                        Ok(Ok(client)) => client,
                        Ok(Err(error)) => {
                            last_transport_error = Some(error.to_string());
                            self.controller.invalidate_client(&forwarded_to);
            ...
            match self.raft.ensure_linearizable(ReadPolicy::ReadIndex).await {
                ...
                Err(error) => {
                    ...
                    let attempt_timeout = deadline
                        .saturating_duration_since(Instant::now())
                        .min(Duration::from_secs(1));
                    let client = match timeout(attempt_timeout, self.controller.get_client(&leader))
                        .await
                    {
                        Ok(Ok(client)) => client,
                        Ok(Err(error)) => {
                            last_error = Some(error.to_string());
                            self.controller.invalidate_client(&leader);
                            sleep(Duration::from_millis(50)).await;
                            continue;
                        }
                        Err(_) => {
